...
def format_weekly_summary_content(parsed_notes: List[Dict[str, Any]], week_num: int, start_str: str, end_str: str) -> str:
    ...
    for t in tickets_processed:
        ...
        if t["is_endpoint"]:
            ...
            lines.append("    * **Đã làm:** Thực hiện kiểm thử nghiệp vụ và giao diện. Áp dụng **Quy trình kiểm thử Feature / UI / Test Execution chuẩn** gồm các giai đoạn:")
            lines.append("        0. **Pre-check with Dastan (Chống duplicate):** Bắt buộc trao đổi trước với Dastan trên Slack để xác nhận tình trạng draft ticket / test cases, tránh bị trùng lặp dữ liệu.")
            lines.append("        1. **Read Request & Create Draft:** Đọc tài liệu yêu cầu Jira & Confluence spec, tạo draft kịch bản bằng lệnh `/create-draft-test-execution`.")

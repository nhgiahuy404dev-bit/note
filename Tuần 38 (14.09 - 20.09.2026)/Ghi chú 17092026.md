...
7. **Bước 7: Tạo Test Run trên TestRail & Thực thi khi Dev deploy Staging (Pass Test)**
   ...
   - **Checklist các bước thực thi kiểm thử chi tiết trên Staging (Execution Steps):**
     - [ ] **1. Chuẩn bị Test Fixtures & Phân quyền:** Đăng nhập tài khoản Admin, cấu hình Grant G1, G2 và gán vai trò/quyền approver cho các User Fixtures (**User A**, **User B**, **User C**, **User D**, **User E**).
     - [ ] **2. Thực thi Test Suite UI (Suite ID `946`):**
       - [ ] **TC-01:** Badge render trên Nav item từ mọi trang (Trading, Live Risk, Blotter).
       - [ ] **TC-02:** Độ chính xác của count (1 - 9) và hiển thị cap `9+` (từ 10 lệnh trở lên, kích thước cố định 18px).
       - [ ] **TC-03:** Không render badge khi count = 0 (xóa DOM, không hiện bare red dot).
       - [ ] **TC-11:** Hiển thị responsive trên màn hình nhỏ / dropdown menu Applications.
       - [ ] **TC-12:** Kiểm tra Accessibility / Screen Reader (`aria-label`, contrast ratio ≥ 4.5:1).
     - [ ] **3. Thực thi Test Suite E2E & Polling (Suite ID `947`):**
       - [ ] **TC-06:** Badge tự động cập nhật sau chu kỳ poll ~60s mà không cần reload trang.
       - [ ] **TC-07 & TC-08:** Kiểm tra cơ chế Polling Gate trên Network tab (`GET /ui/instruction/approvals/count`) và dừng poll khi mất quyền (`eligible: false`).
       - [ ] **TC-04 & TC-05:** Kiểm tra tính Scoping: Không đếm lệnh tự tạo (`I2`), lệnh đã duyệt (`I3`), lệnh ngoài grant (`I4`).
       - [ ] **TC-09:** Optimistic UI: Badge giảm/biến mất ngay lập tức khi click `Approve` hoặc `Reject`.
       - [ ] **TC-10:** Silent Error Handling: Giả lập lỗi API 500/503 không làm vỡ UI, không bắn popup/toast lỗi.
       - [ ] **TC-13:** Deep link: Click badge trên Nav điều hướng chính xác vào tab danh sách lệnh Transfer Tool.
     - [ ] **4. Kiểm tra Hồi quy (Regression Test):**
       - [ ] **RT-01:** Xác nhận indicator cảnh báo chấm đỏ cũ tại header trang Transfer Tool đã được gỡ bỏ triệt để.
     - [ ] **5. Cập nhật Evidence & Ghi nhận kết quả:** Ghi nhận actual result, đính kèm ảnh chụp màn hình / video demo / network log vào từng Test Case trên TestRail Run.
